...
-- 导出  过程 jymanager.getTableColumns 结构
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `getTableColumns`()
BEGIN
        ...
        if not Done then
			...
			if last_tablename<>tablename THEN 
 				if LENGTH(jsonfull) > 0 then
					...
					set jsonfull = concat("var tb_",last_tablename,"=\"",jsonfull, "\";");
				end if;
				set last_tablename=tablename;           
			end if;
...
INSERT INTO `t_ys` (`ys_id`, `ys_type`, `ys_name`, `ys_subname`, `item_name`, `item_year`, `item_type`, `item_buy_way`, `item_tax`, `item_cur_amount`, `item_next_amount`, `item_own_user`, `item_input_datetime`, `item_input_user`, `isvalid`, `commets`) VALUES
	('YS-201605061453000', '生产系统维护预算', '统一支付系统维护', '维护-软件外包预算', '开发预算', '2016', '软件外包', '单一来源', 6, 2000000.00, 0.00, 'qinqiang', '2016-06-08 15:10:42', 'admin', '生效', 'ceshi'),
	('YS-201605130925000', '生产系统维护预算', '物联网PBOSS系统维护', '维护-软件外包预算', '开发预算', '2016', '软件外包', '单一来源', 6, 1000000.00, 0.00, 'wangyl', '2016-05-13 09:26:36', 'zhangsl', '生效', 'ces ');
/*!40000 ALTER TABLE `t_ys` ENABLE KEYS */;


-- 导出  表 jymanager.t_ys_zx 结构
CREATE TABLE IF NOT EXISTS `t_ys_zx` (
  `zx_id` varchar(50) NOT NULL,
  `ys_id` varchar(50) NOT NULL,
  `zx_month` varchar(50) DEFAULT NULL,
  `zx_amount` int(11) DEFAULT NULL,
  `zx_tax` int(11) DEFAULT NULL,
  `zx_input_time` datetime DEFAULT NULL,
  `zx_input_user` varchar(50) DEFAULT NULL,
  `isvalid` varchar(50) DEFAULT NULL,
  `comments` varchar(50) DEFAULT NULL,
  PRIMARY KEY (`zx_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='预算执行记录表';

-- 正在导出表  jymanager.t_ys_zx 的数据：~0 rows (大约)
DELETE FROM `t_ys_zx`;
/*!40000 ALTER TABLE `t_ys_zx` DISABLE KEYS */;
/*!40000 ALTER TABLE `t_ys_zx` ENABLE KEYS */;
